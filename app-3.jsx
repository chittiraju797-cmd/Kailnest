...
// A listing counts as a "featured/Ad" listing if its 7-day paid boost
// (adExpiresAt) hasn't expired yet. Demo/mock listings don't have
// adExpiresAt, so they fall back to their static `featured` flag —
// this keeps existing demo data behaving exactly as before.
function isActiveAd(pg) {
  if (pg && pg.adExpiresAt) return new Date(pg.adExpiresAt) > new Date();
  return !!(pg && pg.featured);
}
...
const FB = {
  ...
  // "For Sale" listings go live immediately (no admin approval wait) since
  // anyone should be able to see them the moment they're posted. Rent
  // listings (PG/Hotel/Apartment/House) still go through admin review.
  async saveListing(listing) {
    try {
      ...
      const ref = await window.db.collection("listings").add({
        ...listing,
        createdAt: new Date().toISOString(),
        verified: listing.category === "Sale" ? true : false,
        featured: false
      });
      return ref.id;
...
function LoginScreen({ onLogin, lang, setLang }) {
  ...
  // Google Sign-in — this account's Google profile is turned into a
  // Kailnest user record. Shared by both the "just redirected back" effect
  // below and, in principle, any other caller.
  const processGoogleUser = async (gUser, roleForSignup) => {
  ...
  // Mobile browsers routinely block or silently swallow signInWithPopup, so
  // Google Sign-in uses signInWithRedirect instead — the whole page
  // navigates to Google and back, rather than opening a popup window. Since
  // that means a full reload, this effect picks the result back up on
  // mount (getRedirectResult resolves with the signed-in user right after
  // the browser returns from Google, and resolves to null on every other
  // normal page load). The role chosen before leaving (tenant/owner) is
  // stashed in sessionStorage since component state doesn't survive the
  // reload.
  useEffect(() => {
    if (!window.auth) return;
    window.auth.getRedirectResult().then(result => {
      if (result && result.user) {
        const roleForSignup = sessionStorage.getItem("kailnest_pending_role") || "tenant";
        sessionStorage.removeItem("kailnest_pending_role");
        setGoogleLoading(true);
        processGoogleUser(result.user, roleForSignup)
          .catch(e => { console.log("Google sign-in error:", e); setAuthError("Google Sign-in failed. Please check your connection and try again."); })
          .finally(() => setGoogleLoading(false));
      }
    }).catch(e => {
      console.log("Google redirect result error:", e);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      if (window.auth) {
        sessionStorage.setItem("kailnest_pending_role", role);
        const provider = new firebase.auth.GoogleAuthProvider();
        provider.addScope('email');
        provider.addScope('profile');
        await window.auth.signInWithRedirect(provider);
        // Execution stops here — the browser navigates to Google's sign-in
        // page. Nothing after this line runs on this page load.
      } else {
  ...
  userSelect: "none", WebkitUserSelect: "none"
            ...
            {step === 1 && (
              <div style={{ textAlign: "center", marginTop: 16, fontSize: 11, color: "#9ca3af", lineHeight: 1.6 }}>
                ...
                <span onClick={() => setShowTerms("terms")} style={{ color: "#6366f1", fontWeight: 600, cursor: "pointer", textDecoration: "underline" }}>Terms of Service</span> and{" "}
                <span onClick={() => setShowTerms("privacy")} style={{ color: "#6366f1", fontWeight: 600, cursor: "pointer", textDecoration: "underline" }}>Privacy Policy</span>.
              </div>
            )}
            ...
            {step === 1 && (
              <div onClick={() => onLogin({ uid: null, name: "Guest", phone: "", role: "guest", provider: "guest", status: "active" })} style={{
                textAlign: "center", marginTop: 14, fontSize: 13, color: "#6366f1", fontWeight: 700, cursor: "pointer"
              }}>Browse without logging in →</div>
            )}
          </>
        )}
      </div>
...
// ─── Legal Documents ─────────────────────────────────────────────────────
// Central registry for every legal/policy document shown via the modal
// below. Adding a new document = adding one entry here.
const LEGAL_DOCS = {
  terms: {
    icon: "📋", label: "Terms of Service", updated: "Last updated: July 1, 2026 · Effective immediately",
    sections: [
  ...
  privacy: {
    icon: "🔒", label: "Privacy Policy", updated: "Last updated: July 1, 2026 · We respect your privacy.",
    sections: [
      ...
      { title: "6. Cookies — Summary", body: "The app uses minimal cookies for session management. See our full Cookie Policy for details." },
      { title: "7. Contact Us", body: "For privacy concerns:\n📧 kailnest5@gmail.com\n🌐 kailnest.in" },
    ],
  },
  ownerAgreement: {
    icon: "🏠", label: "Owner Agreement", updated: "Last updated: August 1, 2026 · Applies to all property owners listing on Kailnest.",
    sections: [
      { title: "1. Who This Applies To", body: "This agreement applies to any individual or entity (\"Owner\") who lists a PG, Hotel, Apartment, House, or For-Sale property on Kailnest." },
      { title: "2. Listing Accuracy", body: "• Photos, price, deposit, and amenities must reflect the real property\n• Contact number provided must be reachable\n• Availability count must be kept up to date" },
      { title: "3. Listing Fee", body: "A flat ₹199 fee applies per listing plan (30-day validity). This fee is for platform visibility only and is non-refundable once the listing goes live." },
      { title: "4. Owner Payments", body: "Rent, deposit, and sale amounts are paid directly by the tenant/buyer to the Owner (via UPI, cash, or bank transfer as agreed between both parties). Kailnest is not a party to this transaction and does not hold or transfer these funds." },
      { title: "5. Prohibited Conduct", body: "• No fake, duplicate, or already-rented listings\n• No discriminatory conditions (caste, religion, etc.)\n• No circumventing the platform to solicit other owners' tenants" },
      { title: "6. Verification", body: "Owners may be asked to submit ID proof (Aadhaar/PAN) and bank details for verification badges. False documents result in permanent suspension." },
      { title: "7. Suspension & Removal", body: "Kailnest may suspend or remove a listing or account for policy violations, repeated complaints, or fraud — with or without prior notice where fraud is suspected." },
      { title: "8. Liability", body: "Owners are solely responsible for the physical condition, safety, and legal compliance (rental agreements, taxes) of their property. Kailnest is only a discovery/listing platform." },
      { title: "9. Contact", body: "Owner support: 📧 kailnest5@gmail.com · 📞 7842375842" },
    ],
  },
  cookies: {
    icon: "🍪", label: "Cookie Policy", updated: "Last updated: August 1, 2026",
    sections: [
      { title: "1. What Are Cookies", body: "Cookies are small pieces of data stored on your device to help the app function and remember your preferences." },
      { title: "2. Cookies We Use", body: "• Session cookies — to keep you logged in\n• Preference cookies — to remember your selected city/language\n• Analytics cookies — anonymized usage data to improve the app" },
      { title: "3. What We Don't Do", body: "We do not use cookies for third-party advertising or to sell your browsing data." },
      { title: "4. Managing Cookies", body: "You can clear cookies anytime through your browser settings. Doing so may log you out or reset your saved preferences." },
      { title: "5. Contact", body: "Questions about cookies: 📧 kailnest5@gmail.com" },
    ],
  },
  copyright: {
    icon: "©️", label: "Copyright Notice", updated: "Last updated: August 1, 2026",
    sections: [
      { title: "1. Ownership", body: "© 2026 Kailnest (kailnest.in). All rights reserved. The Kailnest name, logo, app design, and code are the property of Kailnest and its founder." },
      { title: "2. User-Submitted Content", body: "Property photos and descriptions remain the property of the Owner who uploaded them. By listing, the Owner grants Kailnest a non-exclusive license to display this content on the platform for the purpose of the listing." },
      { title: "3. Third-Party Content", body: "Any stock images or icons used are either licensed or used under fair-use/attribution terms." },
      { title: "4. Reporting Infringement", body: "If you believe content on Kailnest infringes your copyright, email 📧 kailnest5@gmail.com with details and it will be reviewed and removed if valid." },
    ],
  },
  payoutPolicy: {
    icon: "💰", label: "Owner Payout Policy", updated: "Last updated: August 1, 2026",
    sections: [
      { title: "1. How Owners Get Paid", body: "Kailnest does not collect rent, deposit, or sale amounts on behalf of Owners. Tenants/buyers pay Owners directly via UPI, cash, or bank transfer as agreed at the time of booking." },
      { title: "2. Platform Fee", body: "The only amount Kailnest collects is the flat ₹199 listing fee, paid by the Owner to activate/renew a listing. This is not a commission on rent or sale value." },
      { title: "3. UPI Details on Listings", body: "Owners may optionally add their UPI ID or QR code so tenants can pay them directly through the app's booking flow. Kailnest does not hold these funds at any point." },
      { title: "4. Disputes Over Payment", body: "Since payments happen directly between Owner and tenant/buyer, payment disputes are between those two parties. Kailnest can assist by sharing booking records if a complaint is raised." },
      { title: "5. Future Payouts", body: "A platform-managed payout system (e.g. automatic bank transfer of collected rent) is planned but not yet active. This policy will be updated when that feature launches." },
    ],
  },
  commissionModel: {
    icon: "📊", label: "Commission & Fee Model", updated: "Last updated: August 1, 2026",
    sections: [
      { title: "1. Current Model — Flat Listing Fee", body: "Kailnest currently charges a flat ₹199 fee per listing (valid 30 days), regardless of rent, deposit, or sale price. This is not a percentage commission." },
      { title: "2. What's Included", body: "• Listing visibility across Kailnest search & city pages\n• Direct WhatsApp/Call enquiries from tenants/buyers\n• Admin-verified badge eligibility" },
      { title: "3. Optional Boosts", body: "Owners can pay an additional ₹10 for a 7-day \"Ad Boost\" to feature their listing higher in search results." },
      { title: "4. Referral Rewards", body: "Owners who refer other owners earn ₹199 wallet credit per successful referral, redeemable against future listing fees." },
      { title: "5. No Hidden Charges", body: "There is no commission deducted from rent, deposit, or sale amount — those are settled entirely between Owner and tenant/buyer." },
    ],
  },
};
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 2000, display: "flex", alignItems: "flex-end" }}>
      <div style={{ background: "#fff", borderRadius: "22px 22px 0 0", width: "100%", maxHeight: "88vh", display: "flex", flexDirection: "column" }}>
        <div style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", borderRadius: "22px 22px 0 0", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>{doc.icon} {doc.label}</div>
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "#fff", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontWeight: 600 }}>✕ Close</button>
        </div>
        <div style={{ overflowY: "auto", padding: 20, flex: 1 }}>
          <p style={{ color: "#6b7280", fontSize: 12, marginBottom: 16 }}>{doc.updated}</p>
          {doc.sections.map(s => (
...
// ─── Payment History (item 56) ──────────────────────────────────────────
// Shows the logged-in user's own paymentClaims (listing fees, ad boosts,
// etc.) with live status. Purely a read view — approvals still happen in
// the Admin Payments panel.
function PaymentHistoryModal({ user, onClose }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        if (!window.db || !user?.phone) { setLoading(false); return; }
        const snap = await window.db.collection("paymentClaims").where("phone", "==", user.phone).get();
        const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        data.sort((a, b) => new Date(b.claimedAt || 0) - new Date(a.claimedAt || 0));
        setClaims(data);
      } catch (e) {
        console.log("Payment history load error:", e);
        setError(true);
      }
      setLoading(false);
    };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 2000, display: "flex", alignItems: "flex-end" }}>
      <div style={{ background: "#fff", borderRadius: "22px 22px 0 0", width: "100%", maxHeight: "88vh", display: "flex", flexDirection: "column" }}>
        <div style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", borderRadius: "22px 22px 0 0", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>💳 Payment History</div>
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "#fff", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontWeight: 600 }}>✕ Close</button>
        </div>
        <div style={{ overflowY: "auto", padding: 16, flex: 1 }}>
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: 30, color: "#dc2626", fontSize: 13 }}>⚠️ Couldn't load payment history. Check your internet.</div>
          ) : claims.length === 0 ? (
            <div style={{ textAlign: "center", padding: 30, color: "#9ca3af" }}>
              <div style={{ fontSize: 40 }}>💳</div>
              <div style={{ fontSize: 14, marginTop: 8 }}>No payments yet</div>
            </div>
          ) : (
            claims.map(c => {
              const st = statusStyle[c.status] || statusStyle.pending;
              return (
                <div key={c.id} style={{ background: "#f9fafb", borderRadius: 12, padding: 14, marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: "#111" }}>₹{c.amount} {c.purpose === "ad_boost" ? "· Ad Boost" : "· Listing Fee"}</div>
                    <span style={{ background: st.bg, color: st.color, fontSize: 11, fontWeight: 700, padding: "3px 8px", borderRadius: 6 }}>{st.label}</span>
                  </div>
                  <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 6 }}>{c.claimedAt ? new Date(c.claimedAt).toLocaleString("en-IN") : ""}</div>
                  {c.razorpayPaymentId && <div style={{ fontSize: 11, color: "#6b7280", marginTop: 2 }}>ID: {c.razorpayPaymentId}</div>}
                  {c.status === "rejected" && c.rejectReason && (
                    <div style={{ fontSize: 12, color: "#991b1b", marginTop: 6 }}>Reason: {c.rejectReason}</div>
                  )}
                </div>
              );
            })
...
function PGCard({ pg, onView, onBook, isWishlisted, onWishlist }) {
  ...
  return (
    <div style={{
      ...
      border: isActiveAd(pg) ? "2px solid #6366f1" : "1px solid #f3f4f6"
    }}>
      {/* Photo Strip */}
      <div style={{
        ...
        <div style={{
          ...
          {pg.category === "Sale" ? "🟢 Available" : (pg.available === 0 ? "Full" : `${pg.available} beds free`)}
        </div>
      </div>
      ...
      <div style={{ padding: "12px 14px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
          <div>
            ...
            {pg.category !== "Sale" && <span style={{ fontSize: 12, color: "#9ca3af" }}>/month</span>}
            {pg.category !== "Sale" && !!pg.deposit && <div style={{ fontSize: 11, color: "#9ca3af" }}>Deposit: ₹{pg.deposit.toLocaleString()}</div>}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={() => {
              if (pg.category === "Sale") {
                window.open(`https://wa.me/91${pg.phone}?text=${encodeURIComponent(`Hi, I'm interested in "${pg.name}" listed for sale on Kailnest (₹${pg.price.toLocaleString()}).`)}`, "_blank");
              } else {
                onBook(pg);
              }
            }} disabled={pg.category !== "Sale" && pg.available === 0} style={{
              background: (pg.category !== "Sale" && pg.available === 0) ? "#d1d5db" : "linear-gradient(135deg, #6366f1, #8b5cf6)",
              color: (pg.category !== "Sale" && pg.available === 0) ? "#9ca3af" : "#fff", border: "none",
              borderRadius: 10, padding: "8px 16px", fontSize: 13, cursor: (pg.category !== "Sale" && pg.available === 0) ? "not-allowed" : "pointer",
              fontWeight: 700
            }}>{pg.category === "Sale" ? "📞 Contact" : (pg.available === 0 ? "Full" : "Book Now")}</button>
          </div>
        </div>
      </div>
    </div>
...
function PGDetail({ pg, user, onBack, onBook, onChat }) {
  ...
  // "Booking Request" (item 45) — an owner-approval-based alternative to the
  // instant Book Now flow. The tenant sends their intended move-in date and
  // a note; no payment happens here. The owner reviews it (via WhatsApp
  // nudge + bookingRequests collection) and can confirm the actual booking
  ...
  const minReqDate = new Date().toISOString().split("T")[0];

  const submitBookingRequest = async () => {
    if (!user) { alert("Please log in to send a booking request."); return; }
    if (!reqMoveIn) return;
    setSubmittingRequest(true);
    try {
      if (window.db) {
        await window.db.collection("bookingRequests").add({
          pgId: String(pg.id), pgName: pg.name,
          ownerPhone: pg.phone || pg.ownerPhone || "",
          tenantUid: user?.uid || null, tenantName: user?.name || "A tenant", tenantPhone: user?.phone || "",
          moveInDate: reqMoveIn, note: reqNote, status: "pending", requestedAt: new Date().toISOString(),
        });
      }
      const ownerPhone = pg.phone || pg.ownerPhone;
      if (ownerPhone) {
        const msg = encodeURIComponent(`Hi! ${user?.name || "A tenant"} (via Kailnest${user?.phone ? ", +91" + user.phone : ""}) sent a booking request for "${pg.name}" — move-in: ${reqMoveIn}.${reqNote ? " Note: " + reqNote : ""} Please confirm if this works.`);
        window.open(`https://wa.me/91${ownerPhone}?text=${msg}`, "_blank");
      }
    } catch (e) { console.log("Booking request error:", e); }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          ...
          <button onClick={async () => {
            const shareData = {
              title: pg.name,
              text: `Check out "${pg.name}" in ${pg.location} on Kailnest${pg.category === "Sale" ? ` — ₹${pg.price.toLocaleString()}` : ` — ₹${pg.price.toLocaleString()}/month`}`,
              url: "https://kailnest.in",
            };
            try {
              if (navigator.share) {
            ...
            background: "rgba(255,255,255,0.2)", border: "none", color: "#fff",
            borderRadius: 8, padding: "6px 12px", fontSize: 13, cursor: "pointer", fontWeight: 700
          }}>📤 Share</button>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
          ...
          {pg.locationVerified && <Badge text="📍 Location Verified" color="rgba(255,255,255,0.2)" textColor="#fff" />}
          {isActiveAd(pg) && <Badge text="⭐ Featured" color="rgba(255,255,255,0.2)" textColor="#fff" />}
      ...
      <div style={{ padding: 16 }}>
        ...
        {/* Pricing */}
        <div style={{ background: "#fff", borderRadius: 12, padding: 14, marginBottom: 12 }}>
          <div style={{ fontWeight: 700, marginBottom: 10 }}>{pg.category === "Sale" ? "💰 Sale Details" : "💰 Pricing"}</div>
          {(pg.category === "Sale" ? [
            { label: "Sale Price", value: `₹${pg.price.toLocaleString()}` },
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ color: "#9ca3af", fontSize: 12.5 }}>
                {pg.category === "Sale" ? `📞 ${pg.phone || pg.ownerPhone || "Not shared"}` : "Contact shared only after you request it"}
              </div>
            </div>
          </div>
          {pg.category === "Sale" ? (
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <a href={`tel:${pg.phone || pg.ownerPhone}`} style={{
              ...
              <a href={`https://wa.me/91${pg.phone || pg.ownerPhone}?text=${encodeURIComponent(`Hi, I'm interested in "${pg.name}" listed for sale on Kailnest (₹${pg.price.toLocaleString()}).`)}`}
        ...
        {/* Booking Request — owner-approval flow, no payment yet */}
        {pg.category !== "Sale" && pg.available !== 0 && (
          <div style={{ background: "#fff", borderRadius: 12, padding: 14, marginBottom: 12 }}>
            {requestSent ? (
              <div style={{ textAlign: "center", color: "#16a34a", fontSize: 13.5, fontWeight: 600 }}>
                ✅ Request sent! The owner has been notified on WhatsApp.
              </div>
            ) : !showBookingRequestForm ? (
              <button onClick={() => setShowBookingRequestForm(true)} style={{
                width: "100%", padding: "11px", background: "#fff", color: "#6366f1",
                border: "2px solid #6366f1", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer"
              }}>📩 Send Booking Request</button>
            ) : (
              <>
                <div style={{ fontWeight: 700, fontSize: 13.5, marginBottom: 8 }}>📩 Booking Request</div>
                <div style={{ fontSize: 11.5, color: "#9ca3af", marginBottom: 8 }}>No payment now — the owner reviews and confirms with you directly.</div>
                <input type="date" min={minReqDate} value={reqMoveIn} onChange={e => setReqMoveIn(e.target.value)} style={{
                  width: "100%", padding: "10px", borderRadius: 10, border: "1px solid #e5e7eb", fontSize: 13, marginBottom: 8, boxSizing: "border-box"
                }} />
                <textarea value={reqNote} onChange={e => setReqNote(e.target.value)} placeholder="Optional note (e.g. duration, number of people)" rows={2} style={{
                  width: "100%", padding: "10px", borderRadius: 10, border: "1px solid #e5e7eb", fontSize: 13, marginBottom: 8, boxSizing: "border-box", resize: "none", fontFamily: "inherit"
                }} />
                <button onClick={submitBookingRequest} disabled={!reqMoveIn || submittingRequest} style={{
                  width: "100%", padding: "11px",
                  background: (!reqMoveIn || submittingRequest) ? "#d1d5db" : "#1d4ed8",
                  color: "#fff", border: "none", borderRadius: 10, fontSize: 13.5, fontWeight: 700,
                  cursor: (!reqMoveIn || submittingRequest) ? "default" : "pointer"
                }}>{submittingRequest ? "Sending..." : "Send Request"}</button>
              </>
            )}
          </div>
        )}
        ...
        <button onClick={() => {
          ...
          background: (pg.category !== "Sale" && pg.available === 0) ? "#d1d5db" : "linear-gradient(135deg, #6366f1, #8b5cf6)",
          color: (pg.category !== "Sale" && pg.available === 0) ? "#9ca3af" : "#fff", border: "none",
          borderRadius: 14, fontSize: 17, fontWeight: 800, cursor: (pg.category !== "Sale" && pg.available === 0) ? "not-allowed" : "pointer"
        }}>{pg.category === "Sale" ? `📞 Contact Owner — ₹${pg.price.toLocaleString()}` : (pg.available === 0 ? "This PG is Full" : `Book Now — ₹${pg.price.toLocaleString()}/month`)}</button>
      </div>
    </div>
  );
}
...
const RAZORPAY_KEY_ID = "rzp_live_THNC9hH4hIBZQM";

// Loads the Razorpay Checkout script once; reuses it if already loaded.
...
// Opens Razorpay's Checkout popup (Card/UPI/Netbanking) client-side and,
// unlike the plain Payment Button embed, gives us back a real
// razorpay_payment_id on success — so a payment claim can carry proof
// instead of being pure self-report. Admin still approves manually (no
// backend to verify signatures), but can now look the Payment ID up on
// the Razorpay Dashboard before approving.
function payWithRazorpay({ amountRupees, description, prefill, onSuccess, onFailure }) {
  loadRazorpayCheckoutScript().then((loaded) => {
    if (!loaded) { onFailure && onFailure("Razorpay couldn't load. Check your internet."); return; }
    const rzp = new window.Razorpay({
      ...
      theme: { color: "#6366f1" },
      handler: (response) => onSuccess && onSuccess(response.razorpay_payment_id),
      modal: { ondismiss: () => onFailure && onFailure("Payment cancelled") },
    });
    rzp.on("payment.failed", (response) => onFailure && onFailure(response.error?.description || "Payment failed"));
    rzp.open();
  });
}
...
// ₹10 / 7-day "boost" for one listing — pays via Razorpay Checkout (same
// pattern as the owner ₹199 fee), then sets adExpiresAt 7 days out and
// featured:true on that listing. The webhook (Cloud Function) also does
// this independently on payment.captured, so whichever arrives first just
// merges into the same result — no double-charging, no duplicate writes.
function AdBoostButton({ listing, user }) {
  const [loading, setLoading] = useState(false);
  const now = new Date();
  const activeAd = listing.adExpiresAt && new Date(listing.adExpiresAt) > now;
  const daysLeft = activeAd ? Math.max(1, Math.ceil((new Date(listing.adExpiresAt) - now) / 86400000)) : 0;

  const handleBoost = () => {
    setLoading(true);
    payWithRazorpay({
      amountRupees: 10,
      description: "Kailnest Ad Boost — 1 Week — " + listing.name,
      prefill: { name: user?.name, phone: user?.phone },
      notes: { uid: user?.uid || "", phone: user?.phone || "", purpose: "ad_boost", listingId: String(listing.id) },
      onSuccess: async (paymentId) => {
        try {
          if (window.db) {
            const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
            await window.db.collection("listings").doc(String(listing.id)).update({
              adExpiresAt: expiresAt,
            });
            await window.db.collection("paymentClaims").doc(paymentId).set({
              uid: user?.uid || null,
              phone: user?.phone || "",
              amount: 10,
              method: "razorpay",
        ...
        } catch (e) { console.log("Ad boost save error:", e); }
        setLoading(false);
        alert("🎉 Your listing is now featured for 7 days!");
      },
      onFailure: (msg) => { setLoading(false); if (msg !== "Payment cancelled") alert("Payment not completed: " + msg); },
    });
  };
  ...
  if (activeAd) {
    return (
      <div style={{
        ...
        fontSize: 11.5, fontWeight: 700, textAlign: "center", border: "1px solid #fde68a", flexShrink: 0
      }}>📢 Ad active — {daysLeft}d left</div>
    );
  }

  return (
    <button onClick={handleBoost} disabled={loading} style={{
      background: loading ? "#d1d5db" : "linear-gradient(135deg, #f59e0b, #ef4444)",
      color: "#fff", border: "none", borderRadius: 8, padding: "6px 10px",
      fontSize: 11.5, fontWeight: 700, cursor: loading ? "default" : "pointer", flexShrink: 0, whiteSpace: "nowrap"
    }}>{loading ? "Processing..." : "📢 Boost ₹10/wk"}</button>
  );
}
...
function OwnerSubscriptionModal({ onClose, onSelectPlan, user }) {
  ...
  // Razorpay Checkout captures a real razorpay_payment_id on success, which
  // we attach to the claim. Admin still approves manually (no backend to
  // verify signatures/webhooks), but can now confirm the Payment ID exists
  // on the Razorpay Dashboard before approving — instead of trusting a
  // plain "I have paid" self-report with no proof at all.
  const handleRazorpayPay = () => {
    setSubmitting(true);
    payWithRazorpay({
      amountRupees: PRICE,
      description: "Kailnest Owner Listing Fee",
      prefill: { name: user?.name, phone: user?.phone },
      onSuccess: async (paymentId) => {
        try {
          if (window.db) {
            ...
            notifyOwner(`New ₹199 Razorpay payment from ${user?.name || "an owner"} (${user?.phone || "no phone"}) — Payment ID: ${paymentId}. Needs verification.`);
          }
      ...
      onFailure: (msg) => { setSubmitting(false); if (msg !== "Payment cancelled") alert("Payment not completed: " + msg); },
    });
  };
      ...
      <div style={{ background: "#fff", borderRadius: "22px 22px 0 0", width: "100%", padding: 20, maxHeight: "85vh", overflowY: "auto" }}>
        ...
        {paid ? (
          ...
          <>
            ...
            {/* Razorpay Checkout — Card / UPI / Netbanking, captures a real Payment ID */}
            <div style={{ background: "#f9fafb", borderRadius: 14, padding: 14, border: "1px solid #e5e7eb", marginBottom: 14 }}>
              ...
              <button onClick={handleRazorpayPay} disabled={submitting} style={{
                width: "100%", padding: "14px",
                background: submitting ? "#d1d5db" : "linear-gradient(135deg, #6366f1, #8b5cf6)",
                color: "#fff", border: "none", borderRadius: 12,
                fontSize: 15, fontWeight: 800, cursor: submitting ? "default" : "pointer"
              }}>{submitting ? "Opening Razorpay..." : "Pay ₹199 via Razorpay"}</button>
            </div>

            <div style={{ background: "#fffbeb", borderRadius: 10, padding: 10, fontSize: 12, color: "#92400e", border: "1px solid #fde68a", marginBottom: 10 }}>
              💡 After payment, our team verifies your Payment ID and activates your listing — usually within a few hours.
            </div>
...
function OwnerListingForm({ onClose, onSave, user }) {
  ...
  const handleVerifyLocation = () => {
    if (!navigator.geolocation) { setLocationError("Location isn't supported on this device/browser."); return; }
    setLocationError("");
    setGettingLocation(true);
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        setForm(f => ({ ...f, geoLat: pos.coords.latitude, geoLng: pos.coords.longitude, locationVerified: true }));
  ...
  const SALE_FEATURES = ["Parking", "AC", "CCTV", "Security Guard", "Power Backup", "Gym", "Garden", "Borewell", "Lift"];
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: 20 }}>
          ...
          {step === 4 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              ...
              <div style={{ color: "#6b7280", fontSize: 14, marginTop: 6 }}>{form.category === "Sale" ? "Your listing is live now — anyone can see it in For Sale search." : "Your listing has been submitted for review. It will appear in search once approved."}</div>
            </div>
          ) : step === 1 ? (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
                {["PG", "Hotel", "Apartment", "House", "Sale"].map(c => (
                  <button key={c} onClick={() => setForm({...form, category: c, type: c === "Sale" ? "Apartment" : (form.category === "Sale" ? "Boys" : form.type)})} style={{
                    ...
                    {c === "PG" ? "🛏️" : c === "Hotel" ? "🏨" : c === "Apartment" ? "🏢" : c === "House" ? "🏠" : "🏷️"} {c === "Sale" ? "For Sale" : c}
                  </button>
                ))}
              </div>
              ...
              {form.category === "Sale" && (
                <>
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                    {["Apartment", "Villa", "Independent House", "Plot"].map(t => (
                      <button key={t} onClick={() => setForm({...form, type: t})} style={{
              ...
              <label style={labelStyle}>Location Verification (item 68)</label>
              {form.locationVerified ? (
                <div style={{ fontSize: 12.5, color: "#166534", background: "#f0fdf4", borderRadius: 10, padding: "10px 12px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span>📍 GPS location captured ({form.geoLat?.toFixed(4)}, {form.geoLng?.toFixed(4)})</span>
                  <span onClick={handleVerifyLocation} style={{ color: "#1d4ed8", fontWeight: 700, cursor: "pointer" }}>Redo</span>
                </div>
              ) : (
                <>
                  {locationError && <div style={{ fontSize: 11.5, color: "#dc2626", marginBottom: 8 }}>⚠️ {locationError}</div>}
                  <button type="button" onClick={handleVerifyLocation} disabled={gettingLocation} style={{
                    width: "100%", padding: "10px", marginBottom: 12,
                    background: "#eff6ff", color: "#1d4ed8", border: "2px dashed #93c5fd", borderRadius: 10,
                    fontSize: 13, fontWeight: 700, cursor: gettingLocation ? "default" : "pointer"
                  }}>{gettingLocation ? "Getting location..." : "📍 Verify Location (GPS)"}</button>
                </>
              )}
            </>
          ) : step === 2 ? (
            <>
              ...
              {form.category === "Sale" ? (
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input value={form.price} onChange={e => setForm({...form, price: e.target.value})} placeholder="e.g. 3500000" type="number" style={inputStyle} />
                </div>
              ) : (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                  <div>
                    <label style={labelStyle}>Rent/month *</label>
                    <input value={form.price} onChange={e => setForm({...form, price: e.target.value})} placeholder="e.g. 5000" type="number" style={{...inputStyle, marginBottom: 0}} />
                  </div>
                  <div>
                    <label style={labelStyle}>Deposit</label>
                    <input value={form.deposit} onChange={e => setForm({...form, deposit: e.target.value})} placeholder="e.g. 10000" type="number" style={{...inputStyle, marginBottom: 0}} />
                  </div>
                  <div>
                    <label style={labelStyle}>Total Beds *</label>
                    <input value={form.total} onChange={e => setForm({...form, total: e.target.value})} placeholder="e.g. 10" type="number" style={{...inputStyle, marginBottom: 0}} />
                  </div>
                </div>
              )}

              <label style={labelStyle}>Contact Number *</label>
              <input value={form.phone} onChange={e => setForm({...form, phone: e.target.value.replace(/\D/g, "").slice(0, 10)})} placeholder="10-digit mobile number" type="tel" style={inputStyle} />

              <label style={{...labelStyle, marginTop: 12}}>{form.category === "Sale" ? "Property Features" : "Amenities"}</label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                {(form.category === "Sale" ? SALE_FEATURES : ALL_AMENITIES).map(a => (
              ...
              {form.category === "Sale" ? (
                <>
                  ...
                  <textarea value={form.conditions} onChange={e => setForm({...form, conditions: e.target.value})}
                    placeholder="e.g. Clear title, ready to move, negotiable price, loan available." rows={2}
                    style={{...inputStyle, resize: "none"}} />
                </>
              ) : (
                <>
          ...
          <div style={{ padding: "12px 20px 24px", borderTop: "1px solid #f3f4f6" }}>
            ...
            {step < 3 ? (
              <button onClick={() => setStep(step + 1)} disabled={
                (step === 1 && (!form.name || !form.location || !form.city)) ||
                (step === 2 && (!form.price || !form.phone || form.phone.length !== 10 || (form.category !== "Sale" && !form.total)))
              } style={{
                flex: 2, padding: "12px",
                background: ((step === 1 && (!form.name || !form.location || !form.city)) || (step === 2 && (!form.price || !form.phone || form.phone.length !== 10 || (form.category !== "Sale" && !form.total)))) ? "#d1d5db" : "linear-gradient(135deg, #6366f1, #8b5cf6)",
      ...
      {loading ? (
        ...
        filtered.map(o => (
          <div key={o.id} style={{ background: "#fff", borderRadius: 14, border: "1px solid #f3f4f6", marginBottom: 10, overflow: "hidden" }}>
            ...
            {!o.ownerVerified && (o.aadhaarDocUrl || o.panDocUrl || o.bankDocUrl || o.verificationDocUrl) && (
              <div style={{ padding: "0 14px 12px", display: "flex", flexWrap: "wrap", gap: 6 }}>
                {o.aadhaarDocUrl && (
                  <a href={o.aadhaarDocUrl} target="_blank" rel="noreferrer" style={{
                ...
                {o.panDocUrl && (
                  <a href={o.panDocUrl} target="_blank" rel="noreferrer" style={{
                    display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11.5, fontWeight: 700,
                    color: "#1d4ed8", background: "#eff6ff", borderRadius: 8, padding: "6px 10px", textDecoration: "none"
                  }}>🪪 PAN →</a>
                )}
                {o.bankDocUrl && (
                  <a href={o.bankDocUrl} target="_blank" rel="noreferrer" style={{
                    display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11.5, fontWeight: 700,
                    color: "#1d4ed8", background: "#eff6ff", borderRadius: 8, padding: "6px 10px", textDecoration: "none"
                  }}>🏦 Bank Proof →</a>
                )}
                {!o.aadhaarDocUrl && !o.panDocUrl && !o.bankDocUrl && o.verificationDocUrl && (
                ...
                {o.emailVerified && (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11.5, fontWeight: 700, color: "#166534", background: "#dcfce7", borderRadius: 8, padding: "6px 10px" }}>📧 Email verified</span>
                )}
              </div>
            )}
            ...
            {expandedUid === o.id && (
              <div style={{ padding: 12, background: "#f9fafb", borderTop: "1px solid #f3f4f6", fontSize: 12 }}>
                ...
                {(ownerListings[o.id] || []).length === 0 ? (
                  ...
                  <div key={l.id} style={{ color: "#374151", marginBottom: 4 }}>
                    • {l.name || l.title || "Listing"} — {l.location || ""}
                    {l.locationVerified && l.geoLat && l.geoLng && (
                      <a href={`https://maps.google.com/?q=${l.geoLat},${l.geoLng}`} target="_blank" rel="noreferrer" style={{ marginLeft: 6, color: "#1d4ed8", fontWeight: 700 }}>📍 map</a>
                    )}
                  </div>
                ))}
              </div>
        ...
        shown.map(l => (
          <div key={l.id} style={{ background: "#fff", borderRadius: 14, border: "1px solid #f3f4f6", marginBottom: 10, overflow: "hidden" }}>
            <div style={{ padding: 14 }}>
              ...
              {l.locationVerified && l.geoLat && l.geoLng && (
                <a href={`https://maps.google.com/?q=${l.geoLat},${l.geoLng}`} target="_blank" rel="noreferrer" style={{
                  display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 700, color: "#1d4ed8",
                  background: "#eff6ff", borderRadius: 8, padding: "4px 8px", marginTop: 8, textDecoration: "none"
                }}>📍 View GPS location on map →</a>
              )}
...
function PaymentVerificationPanel() {
  ...
  try {
    ...
    if (claim.uid) {
      ...
      await window.db.collection("users").doc(claim.uid).set({ ownerPlanActive: true, ownerPlanName: "Basic", planExpiryDate: expiryDate, role: "owner" }, { merge: true });
  ...
  return (
    <div style={{ padding: 16, fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 16 }}>
        Owners now pay via Razorpay (Card/UPI/Netbanking) and each claim carries a Razorpay Payment ID.
        Look that ID up on your Razorpay Dashboard to confirm it's real before approving. Claims with no Payment ID are self-reported — verify extra carefully.
      </div>
        ...
        shown.map(c => (
          <div key={c.id} style={{ background: "#fff", borderRadius: 14, border: "1px solid #f3f4f6", marginBottom: 10, overflow: "hidden" }}>
            <div style={{ padding: 14 }}>
              ...
              {c.razorpayPaymentId ? (
                <div style={{ fontSize: 11.5, color: "#1d4ed8", marginTop: 6, background: "#eff6ff", borderRadius: 8, padding: "6px 10px", fontFamily: "monospace" }}>
                  💳 Payment ID: {c.razorpayPaymentId}
                </div>
              ) : (
                <div style={{ fontSize: 11.5, color: "#92400e", marginTop: 6, background: "#fffbeb", borderRadius: 8, padding: "6px 10px" }}>
                  ⚠️ No Payment ID — self-reported claim, verify manually before approving
                </div>
              )}
              {c.rejectReason && (
...
export default function PGFinderApp() {
  ...
  const [showTerms, setShowTerms] = useState(null); // "terms" | "privacy" | "ownerAgreement" | "cookies" | "copyright" | "payoutPolicy" | "commissionModel" | null
  ...
  // Owner "Get Verified" — ID proof upload (Aadhaar/PAN) + Bank proof, so
  // admins have documents to review before flipping the ownerVerified flag.
  // Items 63 (Aadhaar), 64 (PAN), 65 (Bank Verification — manual document
  // review, since no penny-drop/automated bank verification API is set up).
  const [docPreviews, setDocPreviews] = useState({ aadhaar: null, pan: null, bank: null });
  const [docUploading, setDocUploading] = useState({ aadhaar: false, pan: false, bank: false });
  const [docErrors, setDocErrors] = useState({ aadhaar: "", pan: "", bank: "" });

  const DOC_FIELD_MAP = { aadhaar: "aadhaarDocUrl", pan: "panDocUrl", bank: "bankDocUrl" };

  const handleDocUpload = async (docType, e) => {
    const file = e.target.files[0];
    if (!file) return;
    setDocErrors(prev => ({ ...prev, [docType]: "" }));
    setDocPreviews(prev => ({ ...prev, [docType]: URL.createObjectURL(file) }));
    if (!window.storage || !user?.uid) {
      setDocErrors(prev => ({ ...prev, [docType]: "Not connected. Check your internet and try again." }));
      return;
    }
    setDocUploading(prev => ({ ...prev, [docType]: true }));
    try {
      const ref = window.storage.ref(`verification/${user.uid}/${docType}_${Date.now()}_${file.name}`);
      await ref.put(file);
      ...
      setUser(prev => ({ ...prev, [field]: url, verificationRequested: true }));
    } catch (err) {
      console.log(`${docType} upload error:`, err);
      setDocErrors(prev => ({ ...prev, [docType]: "Upload failed. Please try again." }));
    }
    setDocUploading(prev => ({ ...prev, [docType]: false }));
  };

  // Email Verification (item 62) — links an email to the phone-authenticated
  // Firebase user (with an internal random password the user never sees)
  // and sends Firebase's built-in verification email. Requires Email/Password
  // sign-in to be enabled once in the Firebase Console for this to work.
  ...
  const handleSendEmailVerification = async () => {
    if (!verifyEmail || !verifyEmail.includes("@")) { setEmailVerifyError("Enter a valid email address."); return; }
    setEmailVerifyError("");
    setSendingEmailVerify(true);
    try {
      const currentUser = window.auth?.currentUser;
      if (!currentUser) throw new Error("Not signed in");
      const tempPassword = "kn_" + Math.random().toString(36).slice(2) + Date.now();
      const credential = firebase.auth.EmailAuthProvider.credential(verifyEmail, tempPassword);
      try {
        await currentUser.linkWithCredential(credential);
      } catch (linkErr) {
        // Email already linked to this account — that's fine, just send the verification email.
        if (linkErr.code !== "auth/provider-already-linked" && linkErr.code !== "auth/email-already-in-use") throw linkErr;
      }
      await currentUser.sendEmailVerification();
      await window.db.collection("users").doc(user.uid).update({ email: verifyEmail });
      setUser(prev => ({ ...prev, email: verifyEmail }));
      setEmailVerifySent(true);
    } catch (err) {
      console.log("Email verification error:", err);
      setEmailVerifyError(err.code === "auth/email-already-in-use" ? "This email is already linked to another account." : "Couldn't send verification email. Please try again.");
    }
    setSendingEmailVerify(false);
  };

  const refreshEmailVerifiedStatus = async () => {
    try {
      const currentUser = window.auth?.currentUser;
      if (!currentUser) return;
      await currentUser.reload();
      if (currentUser.emailVerified) {
        await window.db.collection("users").doc(user.uid).update({ emailVerified: true });
        setUser(prev => ({ ...prev, emailVerified: true }));
      }
    } catch (err) { console.log("Email status refresh error:", err); }
  };
  ...
  const filtered = currentListings.filter(pg => {
    ...
    // "For Sale" listings are shown across all cities together — buyers browsing
    // property to buy shouldn't have to hunt city-by-city for them.
    const matchCity = activeCategory === "Sale" || selectedCity === "All India" || pg.location.toLowerCase().includes(selectedCity.toLowerCase());
  ...
  // Featured listings (paid ads) first, then most-recently reposted/created
  // listings (item 50: Repost listing bumps a listing back to the top of
  // its tier).
  const sortedFiltered = [...filtered].sort((a, b) => {
    const featuredDiff = (isActiveAd(b) ? 1 : 0) - (isActiveAd(a) ? 1 : 0);
    if (featuredDiff !== 0) return featuredDiff;
    const bDate = new Date(b.repostedAt || b.sortDate || b.createdAt || 0).getTime();
    const aDate = new Date(a.repostedAt || a.sortDate || a.createdAt || 0).getTime();
    return bDate - aDate;
  });
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", background: "#f9fafb", minHeight: "100vh", maxWidth: 430, margin: "0 auto" }}>
      ...
      <div style={{ padding: "16px 16px 80px" }}>
          ...
          <>
            ...
            {!activeCategory ? (
              <>
                ...
                {[...PGS, ...HOTELS, ...APARTMENTS, ...HOUSES].filter(l => isActiveAd(l)).map(l => (
        ...
        {tab === "owner" && (
          <>
            ...
            {/* Get Verified — Aadhaar / PAN / Bank proof upload (items 63, 64, 65) */}
              ...
              <div style={{ background: "#fff", borderRadius: 12, padding: 14, marginBottom: 14, border: "1.5px solid #e5e7eb" }}>
                ...
                {user?.verificationRequested ? (
                  <div style={{ fontSize: 12.5, color: "#92400e", background: "#fffbeb", borderRadius: 8, padding: "8px 10px" }}>
                    ⏳ Documents submitted — verification pending admin review. You can still add more documents below.
                  </div>
                ) : (
                  <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 10 }}>Upload your documents so tenants can trust your listings with a verified badge.</div>
                )}
                {[
                  { key: "aadhaar", icon: "🪪", label: "Aadhaar Card", field: "aadhaarDocUrl" },
                  { key: "pan", icon: "🪪", label: "PAN Card", field: "panDocUrl" },
                  { key: "bank", icon: "🏦", label: "Bank Proof (passbook / cancelled cheque)", field: "bankDocUrl" },
                ].map(doc => {
                  const uploadedUrl = user?.[doc.field];
                  return (
                    <div key={doc.key} style={{ marginTop: 10 }}>
                      <div style={{ fontSize: 12.5, fontWeight: 600, color: "#374151", marginBottom: 6 }}>{doc.icon} {doc.label}</div>
                      {uploadedUrl ? (
                        <div style={{ fontSize: 12, color: "#16a34a", background: "#f0fdf4", borderRadius: 8, padding: "8px 10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <span>✅ Uploaded</span>
                          <label style={{ color: "#1d4ed8", cursor: "pointer", fontWeight: 600 }}>
                            Replace
                            <input type="file" accept="image/*" onChange={e => handleDocUpload(doc.key, e)} disabled={docUploading[doc.key]} style={{ display: "none" }} />
                          </label>
                        </div>
                      ) : (
                        <>
                          {docPreviews[doc.key] && (
                            <img src={docPreviews[doc.key]} alt={`${doc.label} preview`} style={{ width: 100, height: 70, objectFit: "cover", borderRadius: 8, border: "1px solid #e5e7eb", marginBottom: 6 }} />
                          )}
                          {docErrors[doc.key] && <div style={{ fontSize: 11.5, color: "#dc2626", marginBottom: 6 }}>⚠️ {docErrors[doc.key]}</div>}
                          <label style={{
                            ...
                            cursor: "pointer", fontSize: 12.5, color: "#1d4ed8", fontWeight: 600
                          }}>
                            {docUploading[doc.key] ? "Uploading..." : `📷 Upload ${doc.label.split(" (")[0]}`}
                            <input type="file" accept="image/*" onChange={e => handleDocUpload(doc.key, e)} disabled={docUploading[doc.key]} style={{ display: "none" }} />
                          </label>
                        </>
                      )}
            ...
            {/* Email Verification (item 62) */}
            {!user?.emailVerified && (
              <div style={{ background: "#fff", borderRadius: 12, padding: 14, marginBottom: 14, border: "1.5px solid #e5e7eb" }}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>📧 Email Verification</div>
                {emailVerifySent ? (
                  <>
                    <div style={{ fontSize: 12.5, color: "#166534", background: "#f0fdf4", borderRadius: 8, padding: "8px 10px", marginBottom: 8 }}>
                      ✅ Verification email sent to {verifyEmail}. Open it and tap the link, then come back and tap below.
                    </div>
                    <button onClick={refreshEmailVerifiedStatus} style={{
                      width: "100%", padding: "9px", background: "#eff6ff", color: "#1d4ed8",
                      border: "1px solid #bfdbfe", borderRadius: 10, fontSize: 12.5, fontWeight: 700, cursor: "pointer"
                    }}>🔄 I've verified — refresh status</button>
                  </>
                ) : (
                  <>
                    <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 8 }}>Add and verify your email for account recovery and important updates.</div>
                    <input type="email" value={verifyEmail} onChange={e => setVerifyEmail(e.target.value)} placeholder="you@example.com" style={{
                      width: "100%", padding: "9px 12px", borderRadius: 10, border: "1px solid #e5e7eb", fontSize: 13, marginBottom: 8, boxSizing: "border-box"
                    }} />
                    {emailVerifyError && <div style={{ fontSize: 11.5, color: "#dc2626", marginBottom: 8 }}>⚠️ {emailVerifyError}</div>}
                    <button onClick={handleSendEmailVerification} disabled={sendingEmailVerify} style={{
                      width: "100%", padding: "9px", background: sendingEmailVerify ? "#d1d5db" : "#1d4ed8", color: "#fff",
                      border: "none", borderRadius: 10, fontSize: 12.5, fontWeight: 700, cursor: sendingEmailVerify ? "default" : "pointer"
                    }}>{sendingEmailVerify ? "Sending..." : "Send Verification Email"}</button>
                  </>
                )}
              </div>
            )}
                  ...
                  <div key={l.id || l.name} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                      <div style={{
                        ...
                        color: "#1d4ed8", borderRadius: 20, padding: "4px 10px", fontSize: 12, fontWeight: 700
                      }}>👁 {l.views || 0}</div>
                      <button onClick={async () => {
                        if (!l.id || !window.db) return;
                        try {
                          const repostedAt = new Date().toISOString();
                          await window.db.collection("listings").doc(String(l.id)).update({ repostedAt, sortDate: repostedAt });
                          setOwnerListings(prev => prev.map(x => (x.id === l.id ? { ...x, repostedAt, sortDate: repostedAt } : x)));
                          alert("🔁 Listing reposted — it's now bumped to the top of search results!");
                        } catch (e) { console.log("Repost error:", e); alert("Couldn't repost right now. Please try again."); }
                      }} style={{
                      ...
                      <AdBoostButton listing={l} user={user} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        {tab === "profile" && (
          <>
            ...
            {[
              ...
              { icon: "🏠", label: "Owner Agreement" },
              { icon: "🍪", label: "Cookie Policy" },
              { icon: "©️", label: "Copyright Notice" },
              { icon: "💰", label: "Owner Payout Policy" },
              { icon: "📊", label: "Commission & Fee Model" },
              ...
              <div key={item.label} style={{
                ...
                if (item.label === "Payment History") setShowPaymentHistory(true);
                ...
                if (item.label === "Owner Agreement") setShowTerms("ownerAgreement");
                if (item.label === "Cookie Policy") setShowTerms("cookies");
                if (item.label === "Copyright Notice") setShowTerms("copyright");
                if (item.label === "Owner Payout Policy") setShowTerms("payoutPolicy");
                if (item.label === "Commission & Fee Model") setShowTerms("commissionModel");
      ...
      {showTerms && <TermsPrivacyModal type={showTerms} onClose={() => setShowTerms(null)} />}
      {showPaymentHistory && <PaymentHistoryModal user={user} onClose={() => setShowPaymentHistory(false)} />}
