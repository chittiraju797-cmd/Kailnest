...
function ForgotPasswordScreen({ onDone, onBack }) {
  ...
  const handleVerifyAnswer = async () => {
    ...
    try {
      ...
      setError("There was a problem sending the OTP. Please try again.");
    }
    setLoading(false);
  };
...
function AdminLoginScreen({ onLogin, onBack }) {
  ...
  // Default fallback credentials — used only until the admin sets a custom
  // password via Settings, at which point Firestore ("adminConfig/credentials")
  // takes over as the source of truth.
  const DEFAULT_USERNAME = "admin";
  const DEFAULT_PASSWORD = "kailnest@2026";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let validPasswordHash = await hashPasswordGlobal(DEFAULT_PASSWORD);

      if (window.db) {
        const configDoc = await window.db.collection("adminConfig").doc("credentials").get();
        if (configDoc.exists) {
          const data = configDoc.data();
          if (data.username) validUsername = data.username;
          if (data.passwordHash) validPasswordHash = data.passwordHash;
        }
      }

      const enteredHash = await hashPasswordGlobal(password);
      if (username.trim() === validUsername && enteredHash === validPasswordHash) {
      ...
      setError("Couldn't verify credentials. Check your connection and try again.");
    } finally {
      setChecking(false);
    }
  };
...
function PGDetail({ pg, user, onBack, onBook }) {
        ...
        {paid ? (
          ...
          <>
            ...
            <div style={{ background: "#f9fafb", borderRadius: 14, padding: 14, border: "1px solid #e5e7eb", marginBottom: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12, color: "#111" }}>💳 Pay ₹199</div>
              ...
              <div style={{ textAlign: "center", marginBottom: 12 }}>
                ...
                />
                <div style={{ fontSize: 11, color: "#6b7280", marginTop: 4 }}>📱 Scan</div>
              </div>
...
function OwnerManagementPanel() {
  ...
  const toggleVerified = async (uid, verified) => {
    if (!window.db) return;
    try {
  ...
  const whatsappVerifiedMsg = (o) => {
    ...
    const text = encodeURIComponent(`Hi ${o.name || ""}! 🎉 Your Kailnest owner account has been verified. You can now list your property. — Kailnest Team`);
    window.open(`https://wa.me/91${o.phone}?text=${text}`, "_blank");
  };

  const updateOwnerStatus = async (uid, status) => {
    if (!window.db) return;
    try {
      await window.db.collection("users").doc(uid).update({ status });
        ...
        filtered.map(o => (
          <div key={o.id} style={{ background: "#fff", borderRadius: 14, border: "1px solid #f3f4f6", marginBottom: 10, overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", borderTop: "1px solid #f3f4f6" }}>
              ...
              <button
                onClick={() => toggleVerified(o.id, !o.ownerVerified)}
              ...
              <button
                onClick={() => updateOwnerStatus(o.id, o.status === "blocked" ? "active" : "blocked")}
...
function AdminNotifications() {
  ...
  const sendBroadcast = async () => {
    ...
    setSending(true);
    try {
      if (!window.db) { alert("Firebase isn't connected right now."); return; }
      ...
      {loading ? (
        ...
        shown.map(c => (
          <div key={c.id} style={{ background: "#fff", borderRadius: 14, border: "1px solid #f3f4f6", padding: 14, marginBottom: 10 }}>
            ...
            {c.status !== "resolved" && (
              <button onClick={() => markResolved(c.id)} disabled={busyId === c.id} style={{
...
// Lets the admin change their own login password. The new password is
// hashed (SHA-256) and stored in Firestore ("adminConfig/credentials"),
// which AdminLoginScreen checks on every login attempt. Every change here
// also notifies the app owner (Firestore log + WhatsApp message) via
// notifyOwner(), same as other sensitive admin actions.
function AdminSettings() {
  ...
  const handleChangePassword = async () => {
    ...
    setSaving(true);
    try {
      if (!window.db) { setMessage({ type: "error", text: "Firebase isn't connected right now." }); return; }

      // Verify current password against Firestore config (fallback to default)
      const DEFAULT_PASSWORD = "kailnest@2026";
      let validHash = await hashPasswordGlobal(DEFAULT_PASSWORD);
      const configRef = window.db.collection("adminConfig").doc("credentials");
      const configDoc = await configRef.get();
      if (configDoc.exists && configDoc.data().passwordHash) {
        validHash = configDoc.data().passwordHash;
      }
      const enteredHash = await hashPasswordGlobal(currentPassword);
      ...
      const newHash = await hashPasswordGlobal(newPassword);
      await configRef.set({ username: "admin", passwordHash: newHash, updatedAt: new Date().toISOString() }, { merge: true });
      ...
      setMessage({ type: "error", text: "Couldn't change password. Please try again." });
    } finally {
      setSaving(false);
    }
  };
...
export default function PGFinderApp() {
  ...
  const isHomeScreen = tab === "search" && !activeCategory && !selectedPG && !bookingPG &&
    ...
    !showCityPicker && !showTerms && !showWishlist;
  ...
  if (selectedPG) return (
    <PGDetail pg={selectedPG} user={user} onBack={() => setSelectedPG(null)} onBook={pg => { setSelectedPG(null); setBookingPG(pg); }} />
  );
        ...
        {tab === "profile" && (
          <>
            ...
            <div style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", borderRadius: 14, padding: 16, marginBottom: 16, color: "#fff" }}>
              ...
              <div style={{ fontSize: 12.5, opacity: 0.9, marginBottom: 10 }}>Share your code with friends. Kailnest rewards are applied by our team once they sign up.</div>
              ...
              <div style={{ fontSize: 12.5, opacity: 0.9, marginBottom: 10 }}>👥 {user?.referralCount || 0} friend{(user?.referralCount || 0) === 1 ? "" : "s"} joined using your code</div>
              <a
