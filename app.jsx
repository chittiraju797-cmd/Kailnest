...
function PaymentVerificationPanel() {
  ...
  try {
    ...
    if (claim.uid) {
      const expiryDate = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
      await window.db.collection("users").doc(claim.uid).update({ ownerPlanActive: true, ownerPlanName: "Basic", planExpiryDate: expiryDate });
          ...
          <>
            ...
            {!activeCategory ? (
              <>
                ...
                {/* InstaRytu cross-promo — Rauu's WhatsApp farmer community for Porumamilla/Kadapa */}
                <a href="https://wa.me/917993315482?text=InstaRytu%20gurinchi%20cheppandi" target="_blank" rel="noreferrer" style={{
                  display: "flex", alignItems: "center", gap: 12, textDecoration: "none",
                  background: "linear-gradient(135deg, #16a34a, #15803d)", borderRadius: 16, padding: 14, marginBottom: 22,
                  boxShadow: "0 2px 8px rgba(0,0,0,0.08)"
                }}>
                  <div style={{ fontSize: 30 }}>🌾</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: "#fff", fontWeight: 800, fontSize: 13.5 }}>InstaRytu — For Farmers</div>
                    <div style={{ color: "#dcfce7", fontSize: 11.5, marginTop: 2 }}>Join our WhatsApp community for machinery rental & produce buying in Porumamilla/Kadapa</div>
                  </div>
