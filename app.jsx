...
function LoginScreen({ onLogin, lang, setLang }) {
    ...
    setTimeout(async () => {
      ...
      if (window.db) {
        try {
          ...
          const existing = await userRef.get();
          if (existing.exists) {
            await userRef.update({ lastLoginAt: userProfile.lastLoginAt, name: userProfile.name, role: userProfile.role });
          } else {
            await userRef.set({ ...userProfile, createdAt: new Date().toISOString() });
          }
        } catch (e) { console.log("User profile save error:", e); }
      }
      setTimeout(() => onLogin({ ...userProfile, upiId, qrPreview }), 1000);
    }, 1000);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #1e1b4b 0%, #312e81 50%, #4338ca 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      ...
      <div onClick={() => setShowAdminLogin(true)} style={{ marginTop: 18, fontSize: 11, color: "#a5b4fc", cursor: "pointer", opacity: 0.7 }}>
        🔐 Admin Login
      </div>
    </div>
  );
...
function AdminLoginScreen({ onLogin, onBack }) {
  ...
  // NOTE: Demo-grade credential check. Replace with real Firebase Auth + admin
  // custom-claim check before production launch.
  const ADMIN_USERNAME = "admin";
  const ADMIN_PASSWORD = "kailnest@2026";
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #0f2a3a 0%, #173c50 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: 24, width: "100%", maxWidth: 380, boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        <button onClick={handleSubmit} style={{
          ...
          color: "#fff", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: "pointer", marginTop: 8
...
function UserManagementPanel() {
  ...
  const deleteUser = async (uid) => {
    if (!window.confirm("ఈ user ని permanently delete చేయాలా? ఈ చర్య వెనక్కి తీసుకోలేరు.")) return;
    if (!window.db) return;
    try {
    ...
    <div style={{ padding: 16 }}>
      {loadError && (
        <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: 12, marginBottom: 14, fontSize: 12, color: "#dc2626" }}>
          ⚠️ Users load కాలేదు. Firebase connection చెక్ చేయండి.
        </div>
      )}
      ...
      <input
        ...
        placeholder="పేరు, phone, email వెతకండి..."
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 30, color: "#9ca3af", fontSize: 13 }}>Users కనపడలేదు.</div>
      ) : (
        filtered.map(u => (
            ...
            {expandedUid === u.id && (
              <div style={{ padding: 12, background: "#f9fafb", borderTop: "1px solid #f3f4f6", fontSize: 12 }}>
                ...
                {(userActivity[u.id]?.listings || []).length === 0 ? (
                  <div style={{ color: "#9ca3af", marginBottom: 8 }}>ఏమీ లేవు</div>
                ...
                {(userActivity[u.id]?.bookings || []).length === 0 ? (
                  <div style={{ color: "#9ca3af" }}>ఏమీ లేవు</div>
...
function OwnerManagementPanel() {
  ...
  const deleteOwner = async (uid) => {
    if (!window.confirm("ఈ owner ని permanently delete చేయాలా? ఈ చర్య వెనక్కి తీసుకోలేరు.")) return;
    if (!window.db) return;
    try {
      await window.db.collection("users").doc(uid).delete();
  ...
  return (
    <div style={{ padding: 16 }}>
      {loadError && (
        <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: 12, marginBottom: 14, fontSize: 12, color: "#dc2626" }}>
          ⚠️ Owners load కాలేదు. Firebase connection చెక్ చేయండి.
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: 30, color: "#9ca3af", fontSize: 13 }}>
          {owners.length === 0 ? "Owner accounts ఇంకా signup కాలేదు." : "Owners కనపడలేదు."}
        </div>
      ) : (
        filtered.map(o => (
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", background: "#f9fafb", minHeight: "100vh", maxWidth: 430, margin: "0 auto", paddingBottom: 40 }}>
      ...
      <div style={{ display: "flex", background: "#fff", borderBottom: "1px solid #f3f4f6" }}>
        {[{ key: "overview", label: "📊 Overview" }, { key: "users", label: "👥 User Management" }, { key: "owners", label: "🏠 Owner Management" }].map(v => (
          <button key={v.key} onClick={() => setView(v.key)} style={{
            flex: 1, padding: "12px", border: "none", background: "transparent", cursor: "pointer",
            fontSize: 12.5, fontWeight: 800, color: view === v.key ? "#0f2a3a" : "#9ca3af",
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ marginTop: 16, textAlign: "center", fontSize: 11, color: "#9ca3af", lineHeight: 1.6 }}>
          More admin tools — Listing Approval,<br />Reports, Analytics — coming next.
        </div>
      </div>
      )}
    </div>
